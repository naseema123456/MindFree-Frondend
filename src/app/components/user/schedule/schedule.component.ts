...
import { userProfileSelector } from '../state/user.selectors';
import { query } from '@angular/animations';
import { SocketIoService } from 'src/app/service/socket-io.service';
import { Observable } from 'rxjs';
import { LoginComponent } from '../login/login.component';
import { compileNgModule } from '@angular/compiler';
// import { IChatRes } from 'src/app/model/message';
...
//   @Input() conversation: Conversation = { _id: '', message: '', conversationId: '', members: [] };
// @Input() currentUserId = '';
...
public chat: boolean = false;
  ...
  //     this.socket.on('recieve-message', (data:any) => {
  //       console.log(data, 'data from recieve');
  ...
  ngOnInit(): void {
    ...
    if (storedId !== null) {
      ...
      this.socket = io(environment.baseUrl, { query: { id :this.activeChatUserId} })
      // Use the listen method instead of on
      // this.socketService.listen('recieve-message').subscribe((data) => {
      //   console.log("ho");
...
this.loadAllAppoinment()
    ...
    const friendId = this.conversation.members.find(m => m !== this.currentUserId);

    if (friendId) {
      this.apiService.getUser(friendId).subscribe(
        (res: any) => {
          this.user = res.user;
        },
        error => {
    ...
    // this.recieveObserver.subscribe(res => {
    //   console.log(res, 'res from recieveObserver');
      
    // })
    // this.socketService.listen('recieve-message').subscribe((data) => {  console.log(data, 'data from recieve'); })
  }
...
openChat(userId: string | undefined): void {
  if (userId) {
    ...
    this.socketService.listen('recieve-message').subscribe((data) => {
      console.log(data);
      if (data.sender && data.messages) {
        // Assuming you have a method named updateMessage
        this.updateMessage(data.sender, data.messages);
      }
    });
  }
}

updateMessage(sender: string, messages: any[]): void {
  console.log(messages, 'data from update message');
  this.messages = [...messages];
  
  // You can also use sender if needed
  console.log(sender, 'sender from update message');
}
